...
describe('daily streak', () => {
    ...
    test('return 1 for 1 day streak when tracking monday wednesday friday', () => {
        expect(
            getDailyStreak([monday], wednesday, trackMondayWednesdayFriday)
        ).toBe(1);
    });

    test('return 0 for 0 day streak when tracking monday wednesday friday', () => {
        expect(
            getDailyStreak(
                [tuesday, thursday],
                saturday,
...
describe('weekly streak', () => {
    ...
    test('return 2 for 2 days streak when you complete them in the same week', () => {
        expect(getWeeklyStreak([monday, wednesday], friday, 2)).toBe(2);
    });

    test('return 2 for 2 days streak when you complete them in the same week with one done today', () => {
        expect(getWeeklyStreak([monday, friday], friday, 2)).toBe(2);
    });

    test('return 0 for 0 days streak when you complete one in the last last week and not in the last week', () => {
        expect(getWeeklyStreak([lastSunday], nextMonday, 1)).toBe(0);
    });

    test('return 1 for 1 day streak when you complete one in the last week', () => {
        expect(getWeeklyStreak([lastSunday], monday, 1)).toBe(1);
    });

    test('return 2 for 2 days streak when you complete one in the last week and one in this week', () => {
...
describe('current week streak', () => {
    ...
    test('return 1 for 1 day streak this week', () => {
        expect(getCurrentWeekStreak([monday], monday)).toBe(1);
    });

    test('return 3 for 3 days streak this week', () => {
        expect(getCurrentWeekStreak([monday, tuesday, friday], saturday)).toBe(
            3
        );
    });

    test('return 2 for 2 days streak this week', () => {
        expect(
